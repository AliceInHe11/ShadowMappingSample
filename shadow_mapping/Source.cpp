...
void renderScene(const Shader& shader, unsigned int texture);
void renderCube();
void renderQuad();
void renderCubeSphere(float radius, int subdivisions);
...
int main()
{
    ...
    // load textures
    // -------------
    unsigned int woodTexture = loadTexture("textures/dirt.bmp");
    unsigned int texture = loadTexture("textures/ceramics.bmp");
    ...
    // lighting info
    // -------------
    glm::vec3 lightPos(0.5f, 25.0f, 0.3f);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        lightPos.y = 5.0 + cos(glfwGetTime()) * 2.0f;
      ...
      //  glBindTexture(GL_TEXTURE_2D, woodTexture);       
        ...
        //glm::mat4 model1 = glm::translate(model1, glm::vec3(3.0f, 3.0f, 3.0f)); // translate it down so it's at the center of the scene
        //model1 = glm::scale(model1, glm::vec3(1.0f, 1.0f, 1.0f));	// it's a bit too big for our scene, so scale it down
        //simpleDepthShader.setMat4("model1", model1);
...
void renderScene(const Shader& shader, unsigned int texture)
{
    ...
    // cubes
    float x = sin(glfwGetTime()) * 4.0f;
    float z = cos(glfwGetTime()) * 3.0f;
    float y = 5.0 + cos(glfwGetTime()) * 5.0f;
    ...
    model = glm::rotate(model, glm::radians(x*100), glm::normalize(glm::vec3(1.0, 0.0, 1.0)));
...
unsigned int cubeSphereVAO = 0;
unsigned int cubeSphereVBO = 0;
...
void renderCubeSphere(float radius, int subdivisions)
{
    if (cubeSphereVAO == 0)
    {
        ...
        for (int i = 0; i <= subdivisions; ++i)
        {
            for (int j = 0; j <= subdivisions; ++j)
            {
                ...
                // Normal vectors (normalized position)
        ...
        glGenVertexArrays(1, &cubeSphereVAO);
        glGenBuffers(1, &cubeSphereVBO);

        glBindBuffer(GL_ARRAY_BUFFER, cubeSphereVBO);
        glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(float), vertices.data(), GL_STATIC_DRAW);

        glBindVertexArray(cubeSphereVAO);
        glEnableVertexAttribArray(0);
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)0);
        glEnableVertexAttribArray(1);
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(3 * sizeof(float)));
    ...
    glBindVertexArray(cubeSphereVAO);
    glDrawArrays(GL_POINTS, 0, (subdivisions + 1) * (subdivisions + 1));
    glBindVertexArray(0);
}
